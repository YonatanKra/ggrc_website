...
class PublicKey extends \WPMailSMTP\Vendor\phpseclib3\Crypt\EC implements \WPMailSMTP\Vendor\phpseclib3\Crypt\Common\PublicKey
{
    ...
    public function verify($message, $signature)
    {
        ...
        if ($this->curve instanceof \WPMailSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards) {
            if ($shortFormat == 'SSH2') {
                list(, $signature) = \WPMailSMTP\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('ss', $signature);
            }
            ...
            try {
                $R = \WPMailSMTP\Vendor\phpseclib3\Crypt\EC\Formats\Keys\PKCS1::extractPoint($R, $curve);
                $R = $this->curve->convertToInternal($R);
            } catch (\Exception $e) {
                return \false;
            }
            $S = \strrev($S);
            $S = new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger($S, 256);
        ...
        if (self::$engines['OpenSSL'] && \in_array($this->hash->getHash(), \openssl_get_md_methods())) {
            $sig = $format != 'ASN1' ? \WPMailSMTP\Vendor\phpseclib3\Crypt\EC\Formats\Signature\ASN1::save($r, $s) : $signature;
