...
abstract class RSA extends \WPMailSMTP\Vendor\phpseclib3\Crypt\Common\AsymmetricKey
{
    ...
    public static function createKey($bits = 2048)
    {
        ...
        if ($num_primes == 2 && $bits >= 384 && self::$defaultExponent == 65537) {
            ...
            if (self::$engines['OpenSSL']) {
                ...
                return \WPMailSMTP\Vendor\phpseclib3\Crypt\RSA::load($privatekeystr);
            }
        }
        static $e;
        if (!isset($e)) {
            $e = new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger(self::$defaultExponent);
        }
        $n = clone self::$one;
        ...
        do {
            for ($i = 1; $i <= $num_primes; $i++) {
                if ($i != $num_primes) {
                    $primes[$i] = \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger::randomPrime($regSize);
                } else {
                    \extract(\WPMailSMTP\Vendor\phpseclib3\Math\BigInteger::minMaxBits($bits));
                    /** @var BigInteger $min
                     *  @var BigInteger $max
                     */
                    ...
                    $primes[$i] = \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger::randomRangePrime($min, $max);
                }
        ...
        // }
        $privatekey = new \WPMailSMTP\Vendor\phpseclib3\Crypt\RSA\PrivateKey();
    ...
    protected static function onLoad($components)
    {
        ...
        if ($components['format'] == \WPMailSMTP\Vendor\phpseclib3\Crypt\RSA\Formats\Keys\PSS::class) {
    ...
    protected function __construct()
    {
        ...
        $this->mgfHash = new \WPMailSMTP\Vendor\phpseclib3\Crypt\Hash('sha256');
    ...
    {
        return new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger($x, 256);
    }
    /**
    ...
    public function withHash($hash)
    {
        ...
        switch (\strtolower($hash)) {
            ...
            case 'sha512/256':
                ...
                break;
            default:
                throw new \WPMailSMTP\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException('The only supported hash algorithms are: md2, md5, sha1, sha256, sha384, sha512, sha224, sha512/224, sha512/256');
        }
    ...
    public function withMGFHash($hash)
    {
        ...
        switch (\strtolower($hash)) {
            ...
            case 'sha512/224':
            case 'sha512/256':
                $new->mgfHash = new \WPMailSMTP\Vendor\phpseclib3\Crypt\Hash($hash);
                break;
            default:
                throw new \WPMailSMTP\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException('The only supported hash algorithms are: md2, md5, sha1, sha256, sha384, sha512, sha224, sha512/224, sha512/256');
        }
    ...
    public function withPadding($padding)
    {
        ...
        if ($numSelected > 1) {
            throw new \WPMailSMTP\Vendor\phpseclib3\Exception\InconsistentSetupException('Multiple encryption padding modes have been selected; at most only one should be selected');
        }
        ...
        if ($numSelected > 1) {
            throw new \WPMailSMTP\Vendor\phpseclib3\Exception\InconsistentSetupException('Multiple signature padding modes have been selected; at most only one should be selected');
        }
