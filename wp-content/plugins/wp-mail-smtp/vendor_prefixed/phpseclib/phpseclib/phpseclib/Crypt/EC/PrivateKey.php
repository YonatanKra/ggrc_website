...
class PrivateKey extends \WPMailSMTP\Vendor\phpseclib3\Crypt\EC implements \WPMailSMTP\Vendor\phpseclib3\Crypt\Common\PrivateKey
{
    ...
    public function multiply($coordinates)
    {
        if ($this->curve instanceof \WPMailSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Montgomery) {
            if ($this->curve instanceof \WPMailSMTP\Vendor\phpseclib3\Crypt\EC\Curves\Curve25519 && self::$engines['libsodium']) {
                return \sodium_crypto_scalarmult($this->dA->toBytes(), $coordinates);
            }
            $point = [$this->curve->convertInteger(new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger(\strrev($coordinates), 256))];
        ...
        $point = \WPMailSMTP\Vendor\phpseclib3\Crypt\EC\Formats\Keys\PKCS1::extractPoint($coordinates, $this->curve);
        $point = $this->curve->multiplyPoint($point, $this->dA);
        if ($this->curve instanceof \WPMailSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards) {
    ...
    public function sign($message)
    {
        ...
        if ($this->curve instanceof \WPMailSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards) {
            ...
            return $shortFormat == 'SSH2' ? \WPMailSMTP\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('ss', 'ssh-' . \strtolower($this->getCurve()), $R . $S) : $R . $S;
        }
        if (self::$engines['OpenSSL'] && \in_array($this->hash->getHash(), \openssl_get_md_methods())) {
            ...
            if ($result) {
                ...
                \extract(\WPMailSMTP\Vendor\phpseclib3\Crypt\EC\Formats\Signature\ASN1::load($signature));
                return $shortFormat == 'SSH2' ? $format::save($r, $s, $this->getCurve()) : $format::save($r, $s);
            }
        }
        $e = $this->hash->hash($message);
        $e = new \WPMailSMTP\Vendor\phpseclib3\Math\BigInteger($e, 256);
    ...
    public function getPublicKey()
    {
        ...
        $key = \WPMailSMTP\Vendor\phpseclib3\Crypt\EC::loadFormat($format, $key);
        if ($this->curve instanceof \WPMailSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Montgomery) {
            return $key;
        }
