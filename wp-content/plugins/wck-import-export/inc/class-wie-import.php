...
class WCK_IE_Import {
	...
	public function json_to_db( $json_content ) {
		...
		if ( $imported_array_from_json !== NULL ) {
			...
			$this->import_messages[$this->j]['message'] = __( 'Uploaded file is not valid json!', 'wie' );
			...
			if( ! empty( $_FILES['cozmos-upload']['tmp_name'] ) ) {
				...
				*/
				...
				if( empty( $this->pbie_import_messages ) ) {
					$this->import_messages[$this->j]['message'] = __( 'Import successfully!', 'wie' );
					$this->import_messages[$this->j]['type'] = 'updated';
					$this->j++;
					flush_rewrite_rules( false );
				}
			} else {
				$this->import_messages[$this->j]['message'] = __( 'Please select a .json file to import!', 'wie' );
				$this->import_messages[$this->j]['type'] = 'error';
				$this->j++;
			}
		}
